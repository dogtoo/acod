...
//2017/03/18 by Nickel 將row重新Register
function advGridGetTableRowNewRegister(row) {
    var NewRequestScenario = {};
    var NewResponseScenario = {};
    // 為各類元件加入編審條件
    $.each(row.find("input,textarea,select,label"), function(key, field) {
        var jfield = $(field);
        var fname = jfield.attr("name");
        advAjaxRegisterField(jfield, fname, NewRequestScenario, NewResponseScenario);
    });
    return [NewRequestScenario, NewResponseScenario];
}

//2017/03/18 by Nickel 將data放回table單一row。
function advGridTableRowResponse(name, scenario, ndx, data) {
    var grid = advGridTable[name];
    var pgrow = grid["pgrow"];
    var begin = grid["viewAt"] * pgrow;
    var i = ndx - begin;
    var row = advGridGetTableRow(grid, i);
    var NewRegister = advGridGetTableRowNewRegister(row);
    $.each(NewRegister[1][scenario], function(key, list) {
        var fname = list[0];
        var jfield = $(list[1]);
        advAjaxSetResponseField(data, fname, jfield, list);
    });
}
        ...
        if (org)
            ...
            if (orgdata) {
                $.each(orgdata, function(key, value) {
                    //2017/03/17 by Nickel Array的型態要解開來比對
                    if (!mod && data[key] instanceof Array) {
                            ...
                            for (var j = 0; j < value.length; j++) {
                                ...
                                if (typeof(value[j]) == "object") {
                                    if (value.hasOwnProperty("key") && value.hasOwnProperty("value")) {
                                        okey = value[j]["key"];
                                        ovalue = value[j]["value"];
                                    } else {
                                        $.each( value[j], function(k, v) {
                                            okey = k;
                                            ovalue= v;
                                            //非設定為key value只接受一筆option
                                            return true;
                                ...
                                if (gkey != okey || gvalue != ovalue)
                                    mod = true;
                            }
                        }
                    }
                    else if (!mod && (data[key] != value))
                        mod = true;
                });
            }
...
function advGridBindCellEvent(grid, jrow, jrowTmpl, ndx, eventName) {
    // private 在樣版中元素指定了 event 事件，或 grid 中指定 event 時，處理 bind event
    var jcols = jrow.find("td,th");
    var jcolsTmpl = jrowTmpl.find("td,th");
    for (var i = 0; i < jcolsTmpl.length; i++) {
        var jcol = $(jcols[i]);
        var jcolTmpl = $(jcolsTmpl[i]);
        // 如果 td 有定 id 則以 id 為主，沒定就用 column index
                        ...
                        for (var k = 0; k < cmdList.length; k++) {
                            ...
                            if (headArgs) {
                                ...
                                if (head == eventName) {
                                    ...
                                    data["ndx"] = ndx;
                                    // bind column
                                    //當change時不能把event放在td所以要找下面的元素
                                    if (eventName == "change") {
                                        jcol = jcol.find("input,textarea,select");
                                    }
                                    jcol.bind(eventName, data, advGridDblClickCol);//借用原本的dblclickcol
                                    bindCol = true;
                                }
                            }
                        }
        ...
        if (!bindCol && grid[eventName]) {
            ...
            data["col"] = id;
            data["ndx"] = ndx;
            jcol.bind(eventName, data, advGridDblClickCol);
        }
    }
}
...
function advGridBuildTableRow(grid, jrow, data, ndx, jrowTmpl) {
    ...
    if ((jrowTmpl && jrowTmpl.find("div[class='acod']")).length > 0 || grid["change"]) {
        advGridBindCellEvent(grid, jrow, jrowTmpl, ndx, "change");
    }
    // 加入鼠標游移列效果
    if (grid["attr"]["hover"])
...
function advGridBuildTable(grid) {
    ...
    if (jfixrow.length > 0)
        jfixrow.empty();
    jtable.empty();
    //2017/03/14 by Nickel 加thead tbody
    if ($(grid["tmpl"]).find("thead")) {
        jtable.append("<thead></thead>");
        if (jfixrow.length > 0)
            jfixrow.append("<thead></thead>");
    }
    if ($(grid["tmpl"]).find("tbody"))
        jtable.append("<tbody></tbody>");
    // 將 template 的標題列轉入 table
    if (grid["head"] > 0) {
        var jhead = advGridGetTmplRow(grid, "head").clone();
        var jheadTemp = advGridGetTmplRow(grid, "head").clone();
        advGridBuildTableShowCol(grid, jhead);
        // 如果設定可排序欄位，就結合 dblclick 事件
        var sortColNdx = grid["sortCols"];
        if (sortColNdx) {
            var jcolList = jhead.find("td,th");
            $.each(sortColNdx, function(k, v) {
                for (var i = 0; i < jcolList.length; i++) {
                    //if (!sortColNdx[i])
                    //    continue;
                    if (sortColNdx[k] != $(jcolList[i]).attr("name"))
                        continue;
                    var data = {};
                    data["colAt"] = i;
                    data["name"] = advGridGetNameSkipArray(sortColNdx[k]);
                    data["grid"] = grid;
        ...
        //測試而已
        if ((jheadTemp && jheadTemp.find("div[class='acod']")).length > 0 || grid["dblclick"]) {
            advGridBindCellEvent(grid, jhead, jheadTemp, -1, "dblclick");
        }
        // 如果有獨立的 fixrow，則標題顯示在 fixrow 上
        if (jfixrow.length > 0) {
            if ($(grid["tmpl"]).find("thead"))
                jfixrow.find("thead").append(jhead);//2017/03/14 by Nickel head加到thead之後
            else
                jfixrow.append(jhead);
        } else {
            if ($(grid["tmpl"]).find("thead"))
                jtable.find("thead").append(jhead);//2017/03/14 by Nickel head加到thead之後
            else
                jtable.append(jhead);
        }
    ...
    for (var i = 0; i < pgrow; i++) {
        ...
        //2017/03/19 by Nickel 有些元素是$root.find的，要先把row放到table再取出來處理
        /*
        ...
        */
        jtable.append(row);
        var nrow = jtable.find("tr:last");
        advGridBuildTableRow(grid, nrow, data, begin + i, rowTmpl);
        advGridBuildTableFixColWidth(grid);
    }
}
...
function advGridShowTotal(grid, func) {
    ...
    if (list) {
        for (var i = 0; i < list.length; i++) {
            ...
            var tag = elem[0].tagName;
            if (tag == "INPUT") {
                //2017/03/15 by Nickel 修正元素為input的錯誤
...
function advGridRegisterItem(jdiv, head, args) {
    ...
    var type = tag == "INPUT" ? field["type"] : null;
    if (((head == "table" || head == "tmpl" || head == "fixrow") && tag == "TABLE") ||
        ((head == "spg" || head == "spgn") && (tag == "SELECT" || tag == "INPUT" || tag == "DIV" || tag == "SPAN" || tag == "LABEL")) ||
        //2017/03/16 by Nickel 其實label 跟 a都可以加click。
        ((head == "fpg" || head == "ppg" || head == "npg" || head == "lpg" || head == "add" || head == "rst") && (tag == "IMG" || tag == "BUTTON" || tag == "LABEL" || tag == "A")) ||
...
// <2015/01/07> ADD BY CHESTER
function advGridDelRow(name, ndx) {
    // public 刪除網格資料內容 (注意，刪除資料內容無法後無法復原！！)
    // name 網格名稱
    // ndx advGridData 的 index (可以為 Integer 或是 Integer 陣列)
    // return null 表示成功，否則為錯誤訊息
    var grid = advGridTable[name];
    var desc = grid["desc"];
    var keep = advGridData[name];
    if (!grid || !grid["valid"])
        return "網格[" + desc + "]不存在";
    if (grid["loading"])
        return "網格[" + desc + "]正在與伺服機交換資料";
    if (!keep)
        return "網格[" + desc + "]無資料內容";
    // 刪除資料內容
    if (Array.isArray(ndx)) {
        for (var i = 0; i < ndx.length; i++) {
            delete keep["new"][ndx[i]];
        }
    } else if (!isNaN(ndx)) {
        delete keep["new"][ndx];
    } else {
        return "傳入參數[ndx]非 Integer 或是 Integer 陣列";
    }
    // 重新整理資料內容
    var newNdx  = 0;
    var newData = {};
    for (var i = 0; i < grid["keepCount"]; i++) {
        if (keep["new"][i]) {
            newData[newNdx] = keep["new"][i];
            newNdx++;
        }
    }
    // 重新設定網格參數
    var dataCount = Object.keys(newData).length;
    var pageCount = Math.ceil(dataCount / grid["pgrow"]);
    grid["viewAt"]           = 0;
    grid["totalPage"]        = pageCount;
    grid["keepPage"]         = pageCount;
    grid["keepCount"]        = dataCount;
    grid["totalCount"]       = dataCount;
    advGridData[name]["new"] = $.extend({}, newData);
    advGridData[name]["org"] = $.extend({}, newData);
    if (grid["attr"]["fullPage"]) {
        // 建立一整頁
        advGridFillFullPage(grid, true);
    }
...
function advGridAddRow(name, rowData) {
    ...
    var table = $(grid["table"]);
    // <2015/08/07> CHESTER 如果有 fixrow 的時候 head 應該要是 0 ，否則 rowCnt 會算錯
    var rowCnt = parseInt((table.find("tr").length - (grid["fixrow"] ? 0 : grid["head"])) / grid["content"]);
    if (isNaN(rowCnt))
        rowCnt = 0;
...
function advGridGetRequest(name, request) {
    ...
    else if (grid["attr"]["reqPage"]) {
        // 只傳本頁
        var begin = grid["viewAt"] * pgrow;
        var totalCount = grid["totalCount"];
        var fullPage = grid["attr"]["fullPage"];
        for (var i = 0; i < pgrow && (fullPage || begin + i < totalCount) ; i++) {
            var row = advGridGetTableRow(grid, i);
            //不太知道為何傳本頁要比對有改過才要傳
            //var data = advGridGetDataRec(grid, begin + i);
            //if (advGridTakeTableRow(grid, row, data, true)) {
...
function advGridRegister(root) {
    ...
    $.each(advGridTable, function (name, grid) {
        if (grid["table"] && grid["tmpl"]) {
            ...
            //2017/03/14 by Nickel 本來就要顯示了加了會影響原table的css
