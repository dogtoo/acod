...
$(document).bind("keydown", function(event){
	...
	//var title = $("#title").html().substr( $("#title").html().indexOf("(")+1, 1 );
	...
	//輸入報單完畢，按entr 比照f6查詢。
	if (keyName == "ENT") {
        // <2015/08/20> CHESTER 沒有 #declNo 元件會有錯誤
		if($('#declNo1').length == 0 || $('#declNo1').val().length != 2 ) return;
		if($('#declNo3').length == 0 || $('#declNo3').val().length != 2 ) return;
		if($('#declNo4').length == 0 || $('#declNo4').val().length != 3 ) return;
		if($('#declNo5').length == 0 || $('#declNo5').val().length != 5 ) return;
