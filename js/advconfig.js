 ...
 *   feememono 字串以繳納証編號 a/bc/d/efghijkl 顯示
 *
...
function advConfigGetNextAcod(jfield) {
    // 2017/03/23 by Nickel 遇到table往上找，要排除tmpl的
    var table = jfield.closest(".tmpl");
    if (table.length > 0 && advConfigRoot)
        return null;
...
function advConfigVerifyText(caller, tag, text, condList) {
    ...
    if (condList) {
        ...
        for (var i = 0; i < condList.length; i++) {
            ...
            //2017/03/08 by Nickel 報單防呆不是從這拿掉的
            /* <2013/11/20> by tony 取消報單號碼的防呆
            */
            ...
            //2016/03/09 by Nickel 這裡跟報單號碼防呆沒關
            /* <2013/11/20> by tony 取消報單號碼的防呆*/
...
function advConfigHasValue(jfield) {
    ...
    if (jfield.length > 0 && jfield[0].tagName == "INPUT") {
        ...
        //有沒有checked應該用.is(":checked")
        if (type == "checkbox")
            has = jfield.is(':checked') ? true : false;
...
function advConfigTextOnBlur(evt) {
    ...
    //2017/03/10 by Nickel 防呆不是從這把 advConfigShowErrorMessage做註解
    // Steve 取消報單號碼的防呆
...
function advConfigParseTransString(line, jfield) {
    ...
    for (var i = 0; i < partList.length; i++) {
        ...
        if (headArgs) {
            //2017/03/08 by Nickel 在Register時就直接轉了
            var text = jfield.val();
            var head = headArgs[1];
            //var args = headArgs[3];
            // 依 head 來解讀條件
            ...
            //2017/03/08 by Nickel 在Register時就直接轉了
            if (head == "upper")
                jfield.val(text.toUpperCase());
            if (head == "lower")
                jfield.val(text.toLowerCase());
            if (head == "cm3") {
                ...
                //2017/03/08 by Nickel 在Register時就直接轉了
                jfield.val(advConfigTransCm3(text));
            }
            if (head == "declno") {
                // 加入一個 class 以便 advAjax 可以判讀
                jfield.addClass("acodConfigDeclno");
                //2017/03/08 by Nickel 在Register時就直接轉了
                jfield.val(advConfigTransDeclno(text));
            }
            if (head == "feememono") {
                // 加入一個 class 以便 advAjax 可以判讀
                jfield.addClass("acodConfigFeememono");
                //2017/03/08 by Nickel 在Register時就直接轉了
                jfield.val(advConfigTransFeememono(text));
            }
        }
    }
    return result;
...
function advConfigSpecDeclNo(jfield, data) {
    ...
    for (var i = 1; i <= 5; i++) {
        ...
        if (njfield.length == 0)
            return;
        //2017/03/19 by Nickel 應該用最後一筆的元素會比較正確...不保證跳的是對的
        //fields[i] = njfield[0];
        fields[i] = njfield[njfield.length-1];
    }
    // 合併欄位加入改變時分解至五個欄位
    ...
    if (nextElem) {
        ...
        for (var i = 0; i < wholeCond.length; i++) {
            ...
            if (cond) {
                ...
                if (dir == "desc") {
                    ...
                    /* 有空再改，應該要可以往前或往後找第一筆
                    var next = {};
                    var autoCmd = cmd.match(/(\w+)(\((\w)\))?/);
                    cmd = autoCmd[1];
                    direction = autoCmd[3];
                    if (!direction || direction == 'all')
                        next = $(advConfigGetSelector(cmd));
                    else if (direction == 'n') {
                        next = $(jfield[0]).next(advConfigGetSelector(cmd));
                    }
                    else if (direction == 'p')
                        next = $(jfield[0]).prev(advConfigGetSelector(cmd));
                    */
                    if (set && next.length > 0) {
                        //2017/03/19 by Nickel 應該用最後一筆的元素會比較正確...不保證跳的是對的
                        //data[dir] = { "cond": false, "curr": jfield, "next": next[0] };
                        data[dir] = { "cond": false, "curr": jfield, "next": next[next.length-1] };
                    }
                }
                else if (dir == "attr") {
                    // attr 可以指定集合
        ...
        if (data["chk"]) {
            ...
            $.each(data["chk"], function(scenario, condList) {
                ...
                if (chkList.length > 0) {
                    ...
                    //2017/03/23 by Nickel 要檢核row來設定的，要重新check元素是否還在
                    if (advConfigRoot) {
                        var checkScenario = [];
                        checkScenario = advConfigCheckScenario[scenario]["list"];
                        advConfigCheckScenario[scenario]["list"] = [];
                        for (i = 0; i < checkScenario.length; i++) {
                            var t = checkScenario[i][0].offsetParent;
                            if (t)
                                advConfigCheckScenario[scenario]["list"].push(checkScenario[i]);
                        }
                    }
                }
            });
        ...
        if (data["attr"]) {
            ...
            $.each(data["attr"], function(scenario, condList) {
                ...
                //2017/03/13 by Nickel 前面沒有把 chk宣告出來要加入下面這段
                if (!advConfigCheckScenario[scenario])
                    advConfigCheckScenario[scenario] = {};
                if (!advConfigCheckScenario[scenario]["list"])
                    advConfigCheckScenario[scenario]["list"] = [];
                ...
                if (advConfigRoot) {
                    var AttrScenario = [];
                    AttrScenario = advConfigAttrScenario[scenario];
                    advConfigAttrScenario[scenario] = [];
                    for (i = 0; i < AttrScenario.length; i++) {
                        var t = AttrScenario[i][0].offsetParent;
                        if (t)
                            advConfigAttrScenario[scenario].push(AttrScenario[i]);
                    }
                }
            });
        }
...
function advConfigPutErrorMessage(errList, alert) {
    ...
    for (var i = 0; i < errList.length; i++) {
        if (i > 0)
            // <2015/07/22> CHESTER 改用逗號隔開
            // advConfigErrorMessage += "\n";
            advConfigErrorMessage += ",";
        advConfigErrorMessage += errList[i];
    }
    if (alert) {
        // <2015/07/22> CHESTER 顯示所有錯誤訊息
        // advConfigShowErrorMessage(errList[0]);
        advConfigShowErrorMessage(advConfigErrorMessage);
        advConfigAlertErrorMessage();
    }
    //2017/03/10 by Nickel 應該要回errMsg提供一個組合錯誤訊息合併的功能
    return advConfigErrorMessage;
}
...
function advConfigCheck(scenario, alert) {
    ...
    if (exoneCheck) {
        //2017/03/10 by Nickel 這裡有group的概念，應該是以group做為一個檢核
        //var cnt = 0;
        //var descList = [];
...
function advConfigModCheck(scenario, alert) {
    ...
    if (advConfigModScenario[scenario]) {
        ...
        for (var i = 0; i < fieldList.length; i++) {
            ...
            if (jfield[0].tagName == "INPUT" && jfield.attr("type").toLowerCase() == "checkbox") {
                //2017/03/13 by Nickel 要用is:checked
                //newValue = jfield.attr("checked") ? "true" : "false";
                newValue = jfield.is(":checked") ? "true" : "false";
            }
