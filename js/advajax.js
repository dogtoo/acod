	...
	if (nextElem) {
		...
		for (var i = 0; i < wholeCond.length; i++) {
			...
			if (cond) {
				...
				//2017/03/08 by Nickel 應該是移去advconfig
				//else if (dir == "tr") {
			    //    // 分解單一 tr 條件
                //    var cmdList = cmd.split(/;/);
...
function advAjaxGetRequestField(data, name, jfield, item) {
    ...
    for (var i = 2; i < item.length; i++) {
            ...
            if (tag == "INPUT") {
                if (type == "text" || type == "password" || type == "hidden")
                    ...
                    */
                    ...
                    //2017/03/17 by Nickel radio新的取法用:checked
                    ...
                    //2017/3/7 by Nickel 是否為checked要用is
                    data[dataName] = jfield.is(':checked') ? true : false;
                }
            }
            else if (tag == "SELECT") {
        ...
        //2017/03/16 by Nickel 有設list也要把屬性回傳
        else if (attr == "list") {
            if (tag == "SELECT") {
                var list = [];
                var options = jfield.prop("options");
                for (var j = 0; j < options.length; j++) {
                    var key = $(options[j]).val();
                    var value = $(options[j]).text();
                    list[j] = {key, value};
                }
                data[dataName] = list;
            }
...
function advAjaxSetResponseField(data, name, jfield, item) {
    ...
    for (var i = 2; i < item.length; i++) {
        ...
        try {
            ...
            //2017/03/08 by Nickel 後端應該也要可以指定不可使用的元素
            else if (attr == "dis") {
                jfield.attr("disabled", data[dataName]);
            }
            //2017/03/07 by Nickel color的c要大寫
            else if (attr == "bgColor") {
                // 設定背景顏色
                jfield.css("background-color", value);
            }
            //2017/03/07 by Nickel 預設選單選項
            else if (attr == "selText") {
                if (tag == "SELECT") {
                    jfield.find("option[value="+data[dataName]+"]").attr("selected", "selected");
                }
                else if (tag == "INPUT" && type == "checkbox") {
                    jfield.attr("checked", data[dataName]);
                }
                else if (tag == "INPUT" && type == "radio") {
                    jfield.filter("input[value="+data[dataName]+"]").attr("checked", true);
                }
            }
            else if (attr == "list") {
                if (tag == "SELECT") {
                    ...
                    if (list && list instanceof Array) {
                        ...
                        for (var j = 0; j < list.length; j++) {
                            ...
                            if (typeof(opt) == "object") {
                                //2017/03/17 by Nickel 接受Array HashMap
